...
def save_complaint(name: str, complaint: str) -> int:
    """Save complaint to database"""
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO complaints (name, complaint) VALUES (?, ?)",
            (name.strip(), complaint.strip())
        )
        conn.commit()
...
def get_complaints(limit: int = 100) -> List[Dict]:
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        rows = conn.execute(
            'SELECT id, name, complaint, timestamp FROM complaints ORDER BY timestamp DESC LIMIT ?',
            (limit,)
        ).fetchall()
        return [dict(row) for row in rows]

def get_recent_complaints_context(limit: int = 3) -> str:
    """Get recent complaints as context for RAG"""
    complaints = get_complaints(limit)
    return "\n".join([f"- {c['complaint']}" for c in complaints])
...
def handle_complaint_chat(api_key: str, model: str):
    """Handle complaint chatbot interface"""
    ...
    with st.expander("💡 วิธีใช้งาน", expanded=False):
        st.markdown(""" 
        1. แชทกับ AI เพื่อบันทึกเรื่องร้องเรียน
        2. AI จะถามชื่อและรายละเอียดเรื่องร้องเรียน
        3. ระบบจะบันทึกอัตโนมัติเมื่อข้อมูลครบ
        """)
    ...
    with st.form("complaint_form", clear_on_submit=True):
        ...
        with col1:
            user_message = st.text_input(
                "ข้อความ",
                placeholder="สวัสดีครับ ผมต้องการร้องเรียน...",
                label_visibility="collapsed"
            )
    ...
    if send_btn and user_message and st.session_state.llm:
        ...
        # Build system prompt
        system_prompt = f"""คุณคือผู้ช่วยที่เป็นมิตรในการรับเรื่องร้องเรียน

ข้อมูลเรื่องร้องเรียนล่าสุด:
{rag_context}

งานของคุณคือเก็บข้อมูล 2 อย่างจากผู้ใช้:
1. ชื่อผู้ร้องเรียน
2. รายละเอียดเรื่องร้องเรียน

ให้ถามข้อมูลทีละรายการตามลำดับด้านบน
แสดงความเห็นใจและเป็นมืออาชีพ
ตอบเป็นภาษาไทยเท่านั้น
"""
        
        messages = [{"role": "system", "content": system_prompt}] + st.session_state.complaint_history
        ...
        conversation_text = " ".join([m['content'] for m in st.session_state.complaint_history])
        
        has_name = any(keyword in conversation_text.lower() for keyword in ['ชื่อ', 'name', 'เรียก'])
        has_complaint = any(keyword in conversation_text.lower() for keyword in ['ร้องเรียน', 'complaint', 'ปัญหา', 'เรื่อง'])
        
        if has_name and has_complaint and len(st.session_state.complaint_history) >= 4:
            user_messages = [m['content'] for m in st.session_state.complaint_history if m['role'] == 'user']
            if len(user_messages) >= 2:
                name = user_messages[0][:100]
                complaint = user_messages[-1][:2000]
                
                try:
                    complaint_id = save_complaint(name, complaint)
